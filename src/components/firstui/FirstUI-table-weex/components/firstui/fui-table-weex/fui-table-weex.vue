<template>
	<!--本文件由FirstUI授权予安徽理工大学（会员ID：  403 4，营业执照号： 1  234000  0 48 53 1 9  95   9   Y）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-table__weex-wrap">
		<scroll-view fixFreezing="true" class="fui-table__nvue-wrap" :class="{'fui-swiper-page':fixFreezing}"
			...
			:style="{width:width+'px',height:height>0 || height!=0?height+unit:'auto'}" @scroll="onScroll">
						...
						<view class="fui-table--td"
							...
							<template v-if="model.type!==3">
								<text class="fui-table--td-text"
									...
									:style="{color:getColColor(model,item[model.prop],index,idx),fontSize:(model.textSize || textSize) +'rpx',width:(model.width+divideW)+'px'}"
									v-if="idx==0 && isTotal && index==tableData.length-1">{{ totalText }}</text>
								<view class="fui-table__checkbox"
									...
									v-else-if="model.type==='selection'" @tap.stop="selectionChange(index)">
									<view class="fui-table__checkmark"
										:style="{borderBottomColor:checkmarkColor,borderRightColor:checkmarkColor}">
									</view>
								</view>
							...
							<template v-else>
								<text v-if="isTotal && index==tableData.length-1"
									:style="{fontSize:textSize +'rpx',fontWeight:'normal'}">-</text>
								<view class="fui-table--tdinner" v-else>
		...
		<template v-if="isDrag">
			<view :ref="`ref_move_${index}`" class="fui-table__td-move" v-for="(item,index) in hData" :key="index"
				:style="{left:(item.leftW-3 - scrollx)+'px',background:index==moveIndex?activeLineColor:'transparent'}"
				@longpress="tdlongtap(index,$event)" @touchstart="touchstart" @touchmove.stop.prevent="touchmove"
				@touchend="touchend(index,$event)" @touchcancel="touchend(index,$event)">
			</view>
		</template>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			touchend(index, e) {
				...
				this.hData.map((item, idx) => {
					item.leftW = item.width + (idx == 0 ? 0 : this.hData[idx - 1].leftW)
				})
				this.totalW += diff
			...
			handleData(vals) {
				...
				if (this.isTotal) {
					...
					this.header.forEach(h => {
						...
						const prop = h.prop
						// 取消计算
						row[prop] = Number(this.getTotalNum(table, prop)) || 'N/A'
					})
					table.push(row)
				}
