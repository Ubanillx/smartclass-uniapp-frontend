<template>
	<!--本文件由FirstUI授权予安徽理工大学（会员ID：4  03 4，营业执照号：1 234 00   0 0   4 8 53 1 99 5   9Y）专用，请尊重知识产权，勿私下传播，违者追究法律责任。-->
	<view class="fui-slideverify__wrap" :style="{width:totalWidth+'px',height:height+'rpx',background:background}">
		...
		<!-- #endif -->
			...
			<view class="fui-sv__slider" :class="{'fui-sv__border-color':!getPassColor && isPass}"
				:style="{width:sliderW+'px',borderColor:isPass?getPassColor: borderColor,background:sliderBgColor}"
			...
			<view class="fui-sv__slider" @touchstart="touchstart" @horizontalpan="touchmove" @touchend="touchend"
				ref="slider" :style="{width:sliderW+'px',borderColor:isPass?getPassColor: borderColor,background:sliderBgColor}">
				<fui-icon name="right" :color="arrowColor" :size="iconSize" v-if="!isPass"></fui-icon>
			...
			<view class="fui-sv__slider"
				...
				:style="{width:sliderW+'px',borderColor:isPass?getPassColor: borderColor,transform:transform,background:sliderBgColor}"
